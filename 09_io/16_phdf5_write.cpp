...
int main (int argc, char** argv) {
  ...
  hsize_t str[2] = {block[0], block[1]};
  hsize_t Nloc[2] = {block[0]/count[0], block[1]/count[1]};
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= Nloc[i];
  ...
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, str, count, Nloc);
  H5Pclose(plist);
